...
| **비율** | **3 : 2** |
| **형식** | PNG 또는 WebP |
| **슬라이더 CSS** | `height: 480px` (PC), `object-fit: cover`, `object-position: center` |

슬라이더 viewport(3:2)와 이미지 비율(3:2)이 같아 `cover`로 꽉 채웁니다.
...
- 모든 텍스트·CTA 버튼은 Safe Area **내부**에 배치
- **슬라이드 상단 소액센터 로고 금지** (헤더와 중복·잘림 방지). **배지/제목부터** 시작
- **슬라이드 내부 CTA 버튼 금지** (좌측 HTML 버튼과 중복). 카톡/상담방법 버튼은 이미지에 넣지 않음
- CTA 버튼(페이지 HTML): **높이 56px**, **radius 14px** 통일
